...
def get_ELSC_target_bug(target, tool):
    ...
    with open(csv_path, "r") as f:
        for line in f:
            ...
            prefix = '-b "' if tool == "smartian" else 'leak kill -target_bug "'
...
def run_fuzzing(benchmark, target, tool, timelimit, opt, cpu_idx):
    ...
    if benchmark == "B-IO" and tool in ["smartian", "SmarTest"]: 
        opt = opt + " " + str(get_IO_target_bug(targ, tool))
    elif benchmark == "B-ELSC" and tool in ["smartian", "SmarTest"]: 
...
def main():
    ...
    outdir = os.path.join(outdir, tool)
    os.makedirs(outdir)
